...
class ApiSentInvoice(models.Model):
    ...
    def action_view_technical_details(self):
        ...
        return {
            ...
            'view_id': self.env.ref(
                'orchida_egy_e_invoicing_trial.view_api_sent_invoice_technical_form'
            ).id,
            'res_id': self.id,
            'target': 'new',
        }
    ...
    def action_open_submission_history(self):
        self.ensure_one()
        action = self.env.ref('orchida_egy_e_invoicing_trial.action_api_sent_invoice').read()[0]
    ...
    def get_ar_inv_id_status(self):
        if not self.env.user.has_group('account.group_account_user'):
            raise AccessError(_('Only accounting users can refresh e-invoice status.'))
        raise_contact_message()
