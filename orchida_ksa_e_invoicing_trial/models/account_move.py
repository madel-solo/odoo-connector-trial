...
class AccountMove(models.Model):
    ...
    orchida_credit_note_reason_code = fields.Selection(
        [(code, f'{code} - {desc}') for code, desc in sorted(_CREDIT_NOTE_REASONS.items())],
        ...
        for move in self:
            ...
            for line in rcm_lines:
                ...
                if not (product.barcode or product.orchida_standard_id):
                    missing.append('GTIN')
                item_type = product.product_tmpl_id.orchida_rcm_item_type or (
                    'S' if product.type == 'service' else 'G'
                )
